...
def send_email(message: str, name: str, recipients: List[str], age: int = None) -> bool:
    ...
    try:
        logger.info(f"Attempting to send birthday reminder for {name} to {total_recipients} recipients")
        ...
        with smtplib.SMTP("smtp.ionos.de", 587, None, 30) as connection:
            ...
            for username in recipients:
                try:
                    ...
                    success_count += 1
                    logger.info(f"Successfully sent birthday reminder for {name} to {username} ({recipient_email})")
    ...
    if success_count > 0:
        logger.info(f"Successfully sent {success_count}/{total_recipients} emails for {name}")
        return True
    else:
...
def check_birthday() -> bool:
    ...
    for person in BIRTHDAY_LIST:
        try:
            ...
            if validation_errors:
                person_name = person.get('fname', 'Unknown') + " " + person.get('lname', 'Unknown')
                logger.error(f"Validation failed for {person_name}: {', '.join(validation_errors)}")
                continue
            
            # Check if birthday is today
            birthday_parts = person["birthday"].split("-")
            if len(birthday_parts) != 3:
                logger.error(f"Invalid birthday format for {person['fname']} {person['lname']}: {person['birthday']}")
                continue
            ...
            if birthday_month == today_month and birthday_day == today_day:
                ...
                try:
                    ...
                    if send_email(message, name, person['send_to'], age):
                        success_count += 1
                        logger.info(f"Birthday reminder processing completed for {name} (age {age})")
                    else:
                        logger.error(f"Failed to send birthday reminder for {name}")
                        
                except Exception as e:
                    logger.error(f"Error processing birthday for {person['fname']} {person['lname']}: {str(e)}")
    ...
    logger.info(f"Birthday check completed. Processed {processed_count} people, sent reminders for {success_count} birthdays.")
...
if __name__ == "__main__":
    try:
        ...
        if success:
            ...
            logger.info("Birthday reminder application completed with no birthdays processed")
    except Exception as e:
